...
// ---------------------------------------------------------------
// Client-side background removal - no server, no ML model download.
// Flood-fills inward from the image border: any border-connected
// region close to the sampled background color becomes transparent.
// Works well for product photos shot on a plain backdrop; will not
// cleanly cut out busy/textured backgrounds (see README).
// ---------------------------------------------------------------
const MAX_DIMENSION = 900;
const COLOR_THRESHOLD = 34;

async function removeBackgroundFloodFill(imageBitmap) {
  ...
  cctx.drawImage(imageBitmap, 0, 0, width, height);
...
async function removeBackgroundML(file) {
  const segmenter = await getSegmenter();
  const output = await segmenter(file);
  return output.toCanvas();
}
...
async function handleFiles(fileList) {
  ...
  try {
    for (const file of files) {
      let garmentCanvas;
      try {
        garmentCanvas = await removeBackgroundML(file);
      } catch (mlErr) {
        console.warn('ML background removal unavailable, using flood-fill fallback:', mlErr);
        const bitmap = await createImageBitmap(file);
        garmentCanvas = await removeBackgroundFloodFill(bitmap);
      }
    ...
    console.error(err);
    showBanner(uploadErrorBanner, 'Something went wrong processing that image. Try a different photo.');
  } finally {
    setUploading(false);
  }
}
